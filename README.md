...
抢课对手速敏感，几个高频操作都有键位（文本框有焦点时不触发，Ctrl+F 除外）：

| 键 | 作用 |
|---|---|
| `F5` | 刷新课程列表 |
| `Ctrl+Enter` | 开抢（仍会弹出开抢前确认） |
| `Esc` | 有弹窗时关弹窗；否则停止抢课 |
| `Ctrl+F` | 聚焦课程搜索框 |

### 关于间隔

「冲刺间隔」只在开抢冲刺窗口内生效，窗口结束后 3 秒线性回落到常规间隔。
真正的限速由内置的请求治理（令牌桶 + 熔断）承担，把间隔调到极小并不会
让实际请求速率无限上升——它只是让你不至于浪费掉限速余量。
